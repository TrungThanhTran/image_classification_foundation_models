...
parser.add_argument('-a', '--arch', metavar='ARCH', default='clip_base_32',
...
parser.add_argument('--model_name', default='clip_base32', type=str,
                    help='url used to set up distributed training')
parser.add_argument('--csv_train', default='', type=str,
                    help='csv  train file')
parser.add_argument('--csv_val', default='', type=str,
                    help='csv  val file')
parser.add_argument('--data_train', default='', type=str,
                    help='image folder  train file')
parser.add_argument('--data_val', default='', type=str,
                    help='image folder  val file')
...
def main():
    ...
    global writer
    writer = SummaryWriter(comment="_clip_base32_freeze_adamw")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch == 'clip_base_32':
        model = TSModel(
            model_name=args.arch,
            global_pool='token',  # no meaning for clip_base
            num_classes=2,
    ...
    if args.dummy:
        ...
        if (args.csv_train is not None) and (args.csv_val is not None):
            pass
            # Define your dataloader here
            # train_dataset = XDataset(args.csv_train,
            #                              args.data_train,
            #                              transforms.Compose([
            ...
            # val_dataset = XDataset(args.csv_val,
            #                            args.data_val,
            #                            transforms.Compose([
...
def save_checkpoint(state, is_best, model_name, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, f'{model_name}_model_best.pth.tar')
